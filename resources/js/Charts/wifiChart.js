...
function renderWifiChart() {
    ...
    // Province = horizontal, Municipality = vertical
    const isHorizontal = el.dataset.type === 'province';

    const safeMax = counts.length ? Math.max(...counts) : 0;

    const { max: yAxisMax, tickAmount } = cleanYAxis(safeMax);
    ...
    const options = {
        chart: {
            type: 'bar',
            height: isHorizontal ? Math.max(360, categories.length * 45) : 360,
            toolbar: { show: false },
            fontFamily: 'inherit',
        ...
        yaxis: {
            min: 0,
            max: isHorizontal ? undefined : yAxisMax,
            tickAmount: isHorizontal ? undefined : tickAmount,
            labels: {
