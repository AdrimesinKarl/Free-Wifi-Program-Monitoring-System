...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5 mb-6">
        <form method="GET" action="{{ route('project-status') }}" class="flex flex-wrap items-center gap-3">

            {{-- Search --}}
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search site name..."
                   class="w-72 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 text-sm focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
            ...
            {{-- Buttons --}}
            <button type="submit"
                    class="px-5 py-2.5 rounded-xl bg-violet-600 text-white text-sm font-medium hover:bg-violet-700 transition">
                Filter
            </button>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800">
            <h2 class="font-semibold text-slate-900 dark:text-slate-100">Project List</h2>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left font-semibold text-slate-600">Site Name</th>
                        <th class="px-6 py-3 text-left font-semibold text-slate-600">Barangay</th>
                        <th class="px-6 py-3 text-left font-semibold text-slate-600">Municipality</th>
                        <th class="px-6 py-3 text-left font-semibold text-slate-600">Region</th>
                        <th class="px-6 py-3 text-left font-semibold text-slate-600">Province</th>
                ...
                @forelse($locations as $loc)
                    <tr class="border-t border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                        ...
                        <td class="px-6 py-4 font-semibold text-slate-900 dark:text-slate-100">{{ $loc->site_name }}</td>
                        <td class="px-6 py-4">{{ $loc->barangay }}</td>
                        <td class="px-6 py-4">{{ $loc->municipality->name }}</td>
                        <td class="px-6 py-4">{{ $loc->municipality->province->region->name }}</td>
                        <td class="px-6 py-4">{{ $loc->municipality->province->name }}</td>
                        <td class="px-6 py-4">
                    ...
                    <tr>
                        <td colspan="7" class="py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="font-medium text-slate-500">No locations found</p>
